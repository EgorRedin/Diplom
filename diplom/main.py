...
from database.models import User, Project, Task, MessageType, Chat
...
async def register(user: schemas.UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    if user_db and not user_db.is_active:
        ...
        user_db.confirmation_code = confirmation_code
        await db.commit()
        ...
        raise HTTPException(status_code=403, detail="Email not confirmed. Please confirm your email to proceed.")
    confirmation_code = ''.join(random.choices(string.digits, k=4))
...
async def create_chat(photo: UploadFile = File(None), name: Optional[str] = Form(),
    ...
    chat_db = await AsyncORM.create_chat(name=name, members=members, photo=photo_chat,
                                         type=type, owner_id=curr_user.id)
    return chat_db


@app.post("/chat/{chat_id}/leave", status_code=status.HTTP_200_OK)
async def leave_chat(chat_id: int, curr_user: User = Depends(security.get_current_user)):
    if curr_user is None:
        raise HTTPException(
            status_code=401,
            detail="Not authenticated"
        )
    await AsyncORM.update_chat_members(chat_id, curr_user.id)


@app.delete("/chat/{chat_id}/delete", status_code=status.HTTP_200_OK)
async def remove_chat(chat_id: int, curr_user: User = Depends(security.get_current_user), db: AsyncSession = Depends(
                      get_db)):
    if curr_user is None:
        raise HTTPException(
            status_code=401,
    ...
    chat_db = await db.execute(select(Chat).options(selectinload(Chat.owner)).where(Chat.id == chat_id))
    chat = chat_db.scalars().first()
    if chat.owner.id != curr_user.id:
        raise HTTPException(
            status_code=404,
            detail="Only owner can remove chat"
        )
    await AsyncORM.remove_chat(chat_id)


@app.post("/chat/{chat_id}/read", status_code=status.HTTP_200_OK)
async def update_readers(chat_id: int, curr_user: User = Depends(security.get_current_user)):
    if curr_user is None:
        raise HTTPException(
            status_code=401,
            detail="Not authenticated"
        )
    await AsyncORM.update_readers(chat_id, curr_user.id)
...
@app.put("/user/delete_photo")
async def remove_photo(curr_user: User = Depends(security.get_current_user), db: AsyncSession = Depends(get_db)):
    if curr_user is None:
        raise HTTPException(
            status_code=401,
            detail="Not authenticated"
        )
    user_db = await db.execute(select(User).where(User.id == curr_user.id))
...
@app.post("/upload_file/{chat_id}", status_code=status.HTTP_201_CREATED)
async def upload_file(chat_id: int, file: UploadFile = File(), curr_user: User = Depends(security.get_current_user)):
    ...
    if file_type and file_type.startswith("image/"):
        ...
        file_type = "file"
    timestamp, msg_id = await AsyncORM.create_message(content=None, file_path=str(file_location),
                                                      sender_id=curr_user.id,
                                                      chat_id=chat_id,
                                                      type=file_type, file_name=str(original_filename),
                                                      file_size=file.size)
...
async def remove_comment(comment_id: int, curr_user: User = Depends(security.get_current_user)):
    if curr_user is None:
        raise HTTPException(
            status_code=401,
            detail="Not authenticated"
        )
    await AsyncORM.remove_comment(comment_id, curr_user.id)
    return 201
...
async def connection(sid, environ, auth: dict):
    ...
    await sio.emit("online", {"user_online": user.id})
...
async def message_handler(sid, data: dict):
    ...
    if message and chat_id:
        try:
            timestamp, msg_id = await AsyncORM.create_message(content=message, file_path=None, sender_id=user_id,
                                                              chat_id=chat_id,
                                                              type="text",
...
async def reaction(sid, data: dict):
    ...
    user_id = session.get("user")
    chat_id = data.get("chat_id")
    reaction_id = data.get("reaction")
    msg_id = data.get("message_id")
    if reaction_id and msg_id and chat_id and user_id:
        try:
            logger.info("Я тут")
            reaction_db_id = await AsyncORM.create_reaction(reaction_id, msg_id, user_id)
            user_db = await AsyncORM.get_user_by_id(user_id)
            chat_db = await AsyncORM.get_single_chat(chat_id)
        except HTTPException and IntegrityError:
            return False
    else:
        return False
    user = schemas.UserSearchResult.from_orm(user_db).dict()
    ...
    await sio.emit("set_reaction", {"user": user, "chat": chat, "message_id": msg_id, "reaction": reaction_id,
                                    "reaction_id": reaction_db_id}, room=f"chat_{chat_id}")


@sio.on("remove_reaction")
async def remove_reaction(sid, data: dict):
    session = await sio.get_session(sid)
    user_id = session.get("user")
    ...
    if not reaction_id:
        return False
    try:
        await AsyncORM.delete_reaction(reaction_id, user_id)
    except HTTPException:
        return False
    try:
        user_db = await AsyncORM.get_user_by_id(user_id)
    ...
    user = schemas.UserSearchResult.from_orm(user_db).dict()
    await sio.emit("remove_reaction", {"reaction_id": reaction_id, "chat_id": chat_id, "user": user}, room=f"chat"
                                                                                                           f"_{chat_id}")
...
async def disconnect(sid):
    ...
    await sio.emit("offline", {"user_offline": user_id})
