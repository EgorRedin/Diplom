...
from sqlalchemy import String, ForeignKey, DateTime, JSON, func, text, ARRAY, Integer
...
class User(Base):
    ...
    reactions: Mapped[list["Reaction"]] = relationship(back_populates="senders", secondary="reaction_users")
    ...
    my_chats: Mapped[list["Chat"]] = relationship(back_populates="owner")
...
class Message(Base):
    ...
    read_id: Mapped[list[int]] = mapped_column(ARRAY(Integer), default=[])
...
class Chat(Base):
    ...
    owner: Mapped["User"] = relationship(back_populates="my_chats")
...
class ReactionUser(Base):
    ...
    reaction_id = mapped_column(ForeignKey("reactions.id", ondelete="CASCADE"), primary_key=True)
    user_id = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), primary_key=True)
...
class Reaction(Base):
    ...
    senders: Mapped[list["User"]] = relationship(back_populates="reactions", secondary="reaction_users")
